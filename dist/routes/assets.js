...
router.get('/roots', asyncHandler(async (req, res) => {
    const [rootAssets] = await pool.query('SELECT * FROM asset WHERE id_parent IS NULL ORDER BY name ASC');
    ...
    const rootsWithHierarchy = await Promise.all(rootAssets.map(async (root) => {
        ...
        const [descendants] = await pool.query(`WITH RECURSIVE asset_hierarchy AS (
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    const [result] = await pool.query('INSERT INTO asset (name, id_parent) VALUES (?, ?)', [name, id_parent]);
    const [newAsset] = await pool.query('SELECT * FROM asset WHERE id = ?', [result.insertId]);
...
router.put('/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { name, id_parent } = req.body;
    await pool.query('UPDATE asset SET name = ?, id_parent = ? WHERE id = ?', [name, id_parent, id]);
    const [updatedAsset] = await pool.query('SELECT * FROM asset WHERE id = ?', [id]);
    res.json(updatedAsset[0]);
}));
// Delete an asset
router.delete('/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    await pool.query('DELETE FROM asset WHERE id = ?', [id]);
    res.status(204).send();
}));
